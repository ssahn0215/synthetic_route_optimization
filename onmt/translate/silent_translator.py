...
def build_default_translator():
    opt = Option(
        models=["./resource/checkpoint/MIT_mixed_augm_model_average_20.pt"],
        alpha=0.,
        beta=0.,
...
class Translator(object):
    ...
    def _fast_translate_batch(self, batch, data, max_length, min_length=0, n_best=1, return_attention=False):
        ...
        alive_seq = torch.full([batch_size * beam_size, 1], start_token, dtype=torch.long, device=device)
        alive_attn = None
        ...
        for step in range(max_length):
            ...
            dec_out, dec_states, attn = self.model.decoder(
                decoder_input, memory_bank, dec_states, memory_lengths=memory_lengths, step=step
            )
            ...
            if is_finished.any():
                ...
                attention = (
                    alive_attn.view(alive_attn.size(0), -1, beam_size, alive_attn.size(-1))
                    ...
                    for j in finished_hyp:
                        ...
                        hypotheses[b].append(
                            (
                                ...
                                attention[:, i, j, : memory_lengths[i]] if attention is not None else None,
                            )
                        )
    ...
    def _translate_batch(self, batch, data):
        ...
        for i in range(self.max_length):
            ...
            for j, b in enumerate(beam):
                if not b.done():
                    if self.mask is not None:
                        b.advance(out[:, j], beam_attn.data[:, j, : memory_lengths[j]], mask[j])
                    else:
